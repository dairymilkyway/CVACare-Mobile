...
const TherapistDashboard = ({ onLogout, onNavigate }) => {
  ...
  const handleCreateReceptive = async () => {
    try {
      const data = await api.therapyAPI.receptive.create(newReceptive);
  ...
  const handleUpdateReceptive = async () => {
    try {
      ...
      const updates = {
        ...
        options: editingReceptive.options,
  ...
  const openEditReceptiveModal = (exercise) => {
    // Transform options from objects to strings for editing
    const transformedExercise = {
      ...exercise,
      options: exercise.options.map(opt => opt.text || opt)
    };
        ...
        >
          <View style={styles.modalContent}>
            ...
            >
              ...
              <TextInput
                style={styles.input}
                value={editingFluency ? editingFluency.exercise_id : newFluency.exercise_id}
                onChangeText={(text) => editingFluency 
                  ? setEditingFluency({...editingFluency, exercise_id: text})
                  : setNewFluency({...newFluency, exercise_id: text})
                }
                placeholder="e.g., FLUENCY_L1_01"
              />

              <Text style={styles.label}>Type</Text>
              <TextInput
                style={styles.input}
                value={editingFluency ? editingFluency.type : newFluency.type}
                onChangeText={(text) => editingFluency 
                  ? setEditingFluency({...editingFluency, type: text})
                  : setNewFluency({...newFluency, type: text})
                }
                placeholder="e.g., Breathing Practice"
              />
              ...
              <TextInput
                ...
                placeholder="e.g., Controlled breathing"
              />
        ...
        >
          <View style={styles.modalContent}>
            ...
            >
              ...
              <TextInput
                style={styles.input}
                value={editingLanguage ? editingLanguage.exercise_id : newLanguage.exercise_id}
                onChangeText={(text) => editingLanguage 
                  ? setEditingLanguage({...editingLanguage, exercise_id: text})
                  : setNewLanguage({...newLanguage, exercise_id: text})
                }
                placeholder="e.g., EXP_L1_01"
              />

              <Text style={styles.label}>Type</Text>
              <TextInput
                style={styles.input}
                value={editingLanguage ? editingLanguage.type : newLanguage.type}
                onChangeText={(text) => editingLanguage 
                  ? setEditingLanguage({...editingLanguage, type: text})
                  : setNewLanguage({...newLanguage, type: text})
                }
                placeholder="e.g., Word Formation"
              />
        ...
        >
          <View style={styles.modalContent}>
            ...
            >
              ...
              <TextInput
                style={styles.input}
                value={editingReceptive ? editingReceptive.exercise_id : newReceptive.exercise_id}
                onChangeText={(text) => editingReceptive 
                  ? setEditingReceptive({...editingReceptive, exercise_id: text})
                  : setNewReceptive({...newReceptive, exercise_id: text})
                }
                placeholder="e.g., REC_L1_01"
              />

              <Text style={styles.label}>Type</Text>
              <TextInput
                style={styles.input}
                value={editingReceptive ? editingReceptive.type : newReceptive.type}
                onChangeText={(text) => editingReceptive 
                  ? setEditingReceptive({...editingReceptive, type: text})
                  : setNewReceptive({...newReceptive, type: text})
                }
                placeholder="e.g., Vocabulary Recognition"
              />
                ...
                <View key={index}>
                  ...
                  <TextInput
                    ...
                    value={editingReceptive 
                      ? (editingReceptive.options[index] || '') 
                      : (newReceptive.options[index] || '')
                    }
                    onChangeText={(text) => {
              ...
              <TextInput
                style={styles.input}
                value={String((editingReceptive ? editingReceptive.correct_answer : newReceptive.correct_answer) + 1)}
                onChangeText={(text) => {
                  const value = parseInt(text) - 1;
                  if (editingReceptive) {
                    setEditingReceptive({...editingReceptive, correct_answer: value >= 0 && value <= 3 ? value : 0});
                  } else {
                    setNewReceptive({...newReceptive, correct_answer: value >= 0 && value <= 3 ? value : 0});
                  }
                }}
                keyboardType="numeric"
                placeholder="1"
        ...
        >
          <View style={styles.modalContent}>
            ...
            >
              ...
              <TextInput
                style={styles.input}
                value={editingArticulation ? editingArticulation.sound_id : newArticulation.sound_id}
                onChangeText={(text) => editingArticulation 
                  ? setEditingArticulation({...editingArticulation, sound_id: text})
                  : setNewArticulation({...newArticulation, sound_id: text})
                }
                placeholder="e.g., s, r, l, k, th"
              />

              <Text style={styles.label}>Level (1-5)</Text>
              <TextInput
                style={styles.input}
                value={String(editingArticulation ? editingArticulation.level : newArticulation.level)}
                onChangeText={(text) => {
                  const value = parseInt(text);
                  if (editingArticulation) {
                    setEditingArticulation({...editingArticulation, level: value >= 1 && value <= 5 ? value : 1});
                  } else {
                    setNewArticulation({...newArticulation, level: value >= 1 && value <= 5 ? value : 1});
                  }
                }}
                keyboardType="numeric"
                placeholder="1"
